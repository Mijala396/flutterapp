...
class _TutorListState extends State<TutorList> {
  ...
  Future<void> getTutorRating(id) async{
    ...
    Navigator.pushNamed(context, '/tutorRating', arguments: {
        "data": data
    });
  }
  ...
  Future<void> getTutorDetails(id) async{
    Response response = await get('http://10.0.2.2:8000/auth/tutor-details/$id/');
    Map Tutor = jsonDecode(response.body);

    Navigator.pushNamed(context, '/tutorDetails',arguments:
    {
      'data':Tutor
    });
  }
